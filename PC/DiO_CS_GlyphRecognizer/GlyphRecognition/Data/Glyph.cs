...
namespace AForge.Vision.GlyphRecognition.Data
{
    ...
    public class Glyph : ICloneable
    {
        ...
        public Bitmap CreateGlyphIcon()
        {
            return CreateGlyphImage(32);
        }

        /// <summary>
        /// Create image for a glyph.
        /// </summary>
        /// <param name="size">Size of the square.</param>
        /// <returns>Rendered image.</returns>
        public Bitmap CreateGlyphImage(int size)
        {
            Bitmap bitmap = new Bitmap(size, size, System.Drawing.Imaging.PixelFormat.Format32bppArgb);

            int cellSize = size / this.Size;
            int glyphSize = this.Size;
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                for (int j = 0; j < size; j++)
                {
                    ...
                    else
                    {
                        ...
                        bitmap.SetPixel(j, i,
                            (this.Data[yCell, xCell] == 0) ? Color.Black : Color.White);
                    }
                }
            }
